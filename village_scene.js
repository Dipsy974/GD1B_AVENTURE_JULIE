class village_scene extends Phaser.Scene {
    constructor() {
    ...
    create() {
        ...
        const tileset = map.addTilesetImage("tileset_v0", "TileSet", 256, 256, 1, 2);
        ...
        const solid = map.createLayer(
            "solid",
            tileset
            ); 
        const intoMaison = map.createLayer(
            "intoMaison",
            tileset
            ); 
        const intoForest = map.createLayer(
            "intoForest",
            tileset
            ); 
        const behind = map.createLayer(
            "behind",
            tileset
            ); 
        ...
        intoMaison.setCollisionByExclusion(-1, true); 
        intoForest.setCollisionByExclusion(-1, true); 
        behind.setDepth(3); 

   
        //INIT VARIABLES DIALOGUE
        ...
        this.physics.add.collider(this.player, intoMaison, this.changeScenetoMaison, null, this);
        this.physics.add.collider(this.player, intoForest, this.changeScenetoForest, null, this);
        this.physics.add.collider(this.player, solid);


        //Initialisation de la caméra et des limites de jeu
        this.cameras.main.setBounds(0, 0, 4352, 2816);
        this.cameras.main.zoom = 0.7;
        this.physics.world.setBounds(0, 0, 4352, 2816);
        //Mise en place de la caméra qui suit le joueur
        this.cameras.main.startFollow(this.player);



        //CREATION DES DIVERS OBJETS EN JEU
        this.ronces = this.physics.add.group({ allowGravity: false, collideWorldBounds: true });
        
        //Parcours le calque d'objets de Tiled pour créer et placer les ronces
        map.getObjectLayer('lesronces').objects.forEach((rc) => {
            const ronce = this.ronces.create(rc.x, rc.y - rc.height/2 ,'ronce').setOrigin(0).setPushable(false);
            ronce.anims.play("wiggle"); 
        ...
        map.getObjectLayer('lathunasse').objects.forEach((thunasse) => {
            const coin = this.coins.create(thunasse.x, thunasse.y - thunasse.height, 'thunasse').setOrigin(0);
        ...
        this.edelweiss = this.physics.add.sprite(3320, 188, 'edelweissUI');

        //COLLIDER JOUEUR AVEC OBJETS
        ...
        this.physics.add.collider(this.player, this.edelweiss, this.collectEdelweiss, null, this);
       
        

        //PNJ TAILLE-HITBOX ET DIALOGUES
        this.npcs = this.physics.add.group();
        this.bucheron = this.npcs.create(600, 1200, 'bucheron');;
        this.bucheron.setPushable(false);
        this.bucheron.body.setSize(140, 130);
        this.bucheron.body.setOffset(55, 110);
        this.bucheron.setDepth(0); 
        this.bucheron.dialogues = [["Hé gamin, je ne peux pas te laisser passer par là !","La forêt est dangeureuse, n'y va pas sans bonne \nraison."],
        ["Tu dois absolument y aller pour ton amoureuse ?","Je vois... dans ce cas, prends donc ce baton","J'ai perdu ma hache mais il devrait faire l'affaire !"],
        ["Tu me rappelles moi quand j'étais jeune.", "Qu'est ce qu'on ferait pas par amour ?..."],
        ["Tu as l'air de savoir t'en servir. Garde la !", "Elle pourrait te servir plus tard."]];    
        this.bucheronHitbox = this.physics.add.sprite(this.bucheron.x, this.bucheron.y, 'hitbox');
        this.bucheronHitbox.setScale(0.7);
        this.bucheronHitbox.setSize(300, 300);
        this.bucheronHitbox.setOffset(-20,60);



        this.girlfriend = this.npcs.create(3600, 2100, 'copine');;
        this.girlfriend.setPushable(false);
        this.girlfriend.setScale(0.7);
        this.girlfriend.body.setSize(140, 130);
        this.girlfriend.body.setOffset(55, 110);
        this.girlfriend.setDepth(0);  
        this.girlfriend.dialogues = [["...","Je sais qu'on devait jouer ensemble mais...","...j'ai vraiment le moral dans les chaussettes \naujourd'hui.","Il n'y a que mes bonbons préférés qui pourraient \nme remonter le moral","Tu voudrais bien aller m'en chercher ?"],
        ["...","Je suis patraque."],
        ["Oooh tu en as trouvé ! Merci infiniment !","*rougit*","Je suis contente que tu sois à mes côtés."]]; 
        this.girlfriendHitbox = this.physics.add.sprite(this.girlfriend.x, this.girlfriend.y, 'hitbox');
        this.girlfriendHitbox.setScale(0.7);
        this.girlfriendHitbox.setSize(200, 200);
        this.girlfriendHitbox.setOffset(20,70);
        

        //COLLIDER JOUEUR ET PNJ
        this.physics.add.overlap(this.player, this.girlfriendHitbox, this.interactionCopine, null, this);
        this.physics.add.collider(this.player, this.girlfriend);
        this.physics.add.overlap(this.player, this.bucheronHitbox, this.interactionBucheron, null, this);
        this.physics.add.collider(this.player, this.bucheron);
    ...
    update() {
        //Bouton d'interaction suit le joueur
        this.ui_button.x = this.player.x + 50;
        this.ui_button.y = this.player.y;

        //Désactive le bouton d'interaction quand il n'overlap plus
        if(!this.player.body.wasTouching.none && this.player.body.touching.none){
            this.ui_button.setVisible(false); 
        }

        //Controles joueur et attaque que quand dialogue pas actif
        if(!this.dialogActive){
            ...
            if(this.player.hasBaton || this.player.hasHache){
                if(this.player.isAttacking){
                    ...
                    if(this.player.dir == "up"){
                        ...
                        //this.attackHitBox.anims.play('attack_up');  
                    }
            ...
            //FREEZE DES DEPLACEMENTS/ANIMS JOUEURS SI DIALOGUE ACTIF
            ...
            //UPDATE POSITION PAR RAPPORT AUX PNJ
    ...
    //RAMASSE OBJET
    collectEdelweiss(player, fleur){
        fleur.destroy(); 
        this.player.hasEdelweiss = true; 
        this.ui_edelweiss.setVisible(true); 
    }
    ...
    changeScenetoForest(player, trigger){
        if(player.hasBaton || player.hasHache){
            ...
            this.discussion(this.bucheron.dialogues[0]); 
        }
        
    }

    changeScenetoMaison(player, trigger){
    ...
    //INTERACTION PNJ
    interactionCopine(player, npc){
        this.ui_button.setVisible(true); 
        if(!this.dialogActive){
            const isEJustDown = Phaser.Input.Keyboard.JustDown(this.keyE);
            if(isEJustDown){
                //SEQUENCES DE DIALOGUES DIFFERENTES SELON STATUT
                if(this.player.hasBonbons){
                    this.discussion(this.girlfriend.dialogues[2]);
                }
                else if(this.player.questGiven){
                    this.discussion(this.girlfriend.dialogues[1]);
                }
                else{
                    this.discussion(this.girlfriend.dialogues[0]);
                    this.player.questGiven = true; 
                } 
            }
        }
    ...
    interactionBucheron(player, npc){
        this.ui_button.setVisible(true); 
        if(!this.dialogActive){
                const isEJustDown = Phaser.Input.Keyboard.JustDown(this.keyE);
                if(isEJustDown){
                    if(this.player.hasHache){
                        this.discussion(this.bucheron.dialogues[3]);
                    }
                    else if(this.player.hasBaton){
                        this.discussion(this.bucheron.dialogues[2]);
                    }
                    else if(this.player.recetteGiven){
                        this.player.hasBaton = true; 
                        this.ui_baton.setVisible(true); 
                        this.discussion(this.bucheron.dialogues[1]);
                        this.player.currentAnims = ["run_left_baton","run_up_baton","run_right_baton","run_down_baton","idle_left_baton","idle_up_baton","idle_right_baton","idle_down_baton"];
                    }
                    else{ 
                        this.discussion(this.bucheron.dialogues[0]);
                    } 
   ...
   //COLLISION AVEC ENNEMI, BLESSE JOUEUR
    ...
    //MORT JOUEUR, LANCEMENT SCENE VILLAGE
    die(){
